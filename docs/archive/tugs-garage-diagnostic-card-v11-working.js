class TugsGarageDiagnosticCard extends HTMLElement {
  setConfig(config) {
    this.config = {
      ...
      status_rules: Array.isArray(config.status_rules) ? config.status_rules : [],
  ...
  getAutomaticDeviceStatus() {
    const sensors = this.config.sensors || [];
    const totalSensors = sensors.length;

    const goodSensors = sensors.filter((sensor) => {
      const stateObj = this._hass.states[sensor.entity];
      return stateObj && this.isGoodState(stateObj.state);
    }).length;

    if (goodSensors === totalSensors) return "online";
    if (goodSensors === 0) return "offline";
    return "unknown";
  }

  getAutomaticSystemStatus() {
    const rules = this.config.status_rules || [];
    let status = "normal";

    rules.forEach((rule) => {
      const sensor = this.config.sensors.find((s) => s.key === rule.key);
      if (!sensor) return;
      ...
      const faultLow = rule.fault_low;
      const faultHigh = rule.fault_high;
      const warningLow = rule.warning_low;
      const warningHigh = rule.warning_high;

      if (
        (faultLow !== undefined && value < faultLow) ||
        (faultHigh !== undefined && value > faultHigh)
      ) {
        status = "fault";
        return;
      }
  ...
  getDeviceBadge(deviceStatus) {
    ...
    return badges[cleanStatus] || badges.unknown;
  }

  render() {
    if (!this.config || !this._hass) return;

    const systemStatus = this.getAutomaticSystemStatus();
    const deviceStatus = this.getAutomaticDeviceStatus();

    const statusBadge = this.getStatusBadge(systemStatus);
    const deviceBadge = this.getDeviceBadge(deviceStatus);
